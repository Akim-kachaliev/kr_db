...
User* AuthController::login(const QString& login, const QString& password) {
    ...
    if (!u) {
        QString dbErr = DatabaseManager::getInstance()->lastError();
        if (!dbErr.isEmpty()) {
            err_ = "Ошибка БД: " + dbErr;
        } else {
            err_ = QString("Пользователь '%1' не найден в БД. Проверьте содержимое таблицы List.").arg(login);
        }
        return nullptr;
    }
    ...
    if (u->passwordHash() != password) {
        ...
        if (u->passwordHash() != inputHash) {
            err_ = QString("Неверный пароль для пользователя '%1'.\nВ БД сохранено: '%2'\nВведённый пароль: '%3'\nХэш введённого: '%4'")
                       .arg(login, u->passwordHash(), password, inputHash);
            delete u;
            return nullptr;
        }
    }
